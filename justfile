...
build:
    ...
    ros_packages="tbai_ros_utils tbai_ros_msgs tbai_ros_core grid_map"
...
ros-build-all: build
    ...
    echo 'export GAZEBO_PLUGIN_PATH=$GAZEBO_PLUGIN_PATH:$CONDA_PREFIX/lib' >> ../../install/setup.sh

# Build go2 ROS packages
ros-build-go2: 
    #!/usr/bin/env bash
    cd ~/tbai_ws
    MAKEFLAGS="-j 4" colcon build --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=Release --executor sequential --packages-up-to tbai_ros_deploy_go2_rl

# Fresh install go2 environment
fresh-install-go2: clean clone-tbai build-tbai ros-build-go2 # install-tbai-safe
    #!/usr/bin/env bash
    cd ~/tbai_ws
    MAKEFLAGS="-j 4" colcon build --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=Release --executor sequential --packages-up-to elevation_mapping elevation_mapping_cupy hesai_ros_driver realsense2_camera
    echo "All good 🤗"
...
fresh-install-go2-gpu-free: clean clone-tbai build-tbai ros-build-go2 # install-tbai-safe
    #!/usr/bin/env bash
    cd ~/tbai_ws
    MAKEFLAGS="-j 4" colcon build --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=Release --executor sequential --packages-up-to elevation_mapping realsense2_camera hesai_ros_driver
    echo "All good 🤗"
...
fresh-install-all: clean clone-tbai build-tbai ros-build-all # install-tbai-safe
    #!/usr/bin/env bash
    cd ~/tbai_ws
    MAKEFLAGS="-j 4" colcon build --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=Release --executor sequential --packages-up-to elevation_mapping elevation_mapping_cupy
    echo "All good 🤗"
